//
//  AudioToolBoxDecoder.m
//  DecodeByVideoTool
//
//  Created by 陈剑 on 2022/1/4.
//

#import "AudioToolBoxDecoder.h"
...
@interface AudioToolBoxDecoder ()

@end

@implementation AudioToolBoxDecoder
...
- (void)decodeAudioWithSourceBuffer:(void *)sourceBuffer sourceBufferSize:(UInt32)sourceBufferSize completeHandler:(void(^)(AudioBufferList *destBufferList, UInt32 outputPackets,AudioStreamBasicDescription * outputBasicDescriptions))completeHandler {
...
- (AudioConverterRef)configureDecoderBySourceFormat:(AudioStreamBasicDescription)sourceFormat destFormat:(AudioStreamBasicDescription *)destFormat destFormatID:(AudioFormatID)destFormatID sampleRate:(float)sampleRate isUseHardwareDecode:(BOOL)isUseHardwareDecode {
    ...
    [AudioToolBoxDecoder printAudioStreamBasicDescription:sourceFormat];
    printf("Destination File format:\n");
    [AudioToolBoxDecoder printAudioStreamBasicDescription:destinationFormat];
...
//    AVAudioFormat *format = [[AVAudioF　ormat alloc]initWithStreamDescription:&sourceFormat];
//    AVAudioPCMBuffer *pcmBuffer = [[AVAudioPCMBuffer alloc]initWithPCMFormat:format frameCapacity:1024];
    ...
    if (status != noErr) {
        ...
        if (completeHandler) {
            completeHandler(&fillBufferList, ioOutputDataPackets, &sourceFormat);
        }
    }
}
